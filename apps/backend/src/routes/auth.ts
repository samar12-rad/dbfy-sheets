...
router.post('/google/start', authenticateUser, (req: Request, res: Response) => {
...
router.get('/google/callback', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const sheetMetadata = await sheetsClient.spreadsheets.get({ spreadsheetId });
        const sheetName = sheetMetadata.data.properties?.title || 'Untitled Sheet';
        ...
        try {
            ...
            await connection.query(
                ...
                [userId, result.insertId, 'SHEET_CONNECT', 'SHEET', result.insertId, JSON.stringify({ external_sheet_id: spreadsheetId })]
            );
